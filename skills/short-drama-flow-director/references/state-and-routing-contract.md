...
`artifact_type` 使用 `PLOT`、`STORYBOARD_TABLE`、`STORYBOARD_PROMPT`、`STORYBOARD_IMAGE`、`APPROVED_STORYBOARD_SET`、`VIDEO_PROMPT` 或 `DELIVERY_PACKAGE`。同一稳定 `artifact_id` 只能有一个 `current: true` 的版本。`full_id` 必须等于稳定 ID 加版本；`STALE` 与 `stale: true` 必须一致。
...
|---|---|---|
| P1 | `PLOT` | 当前可读取剧本；受影响范围无未解决来源冲突 |
| P2 | `STORYBOARD_TABLE` | 当前 `PLOT: PASS` |
| P3 | `STORYBOARD_PROMPT` | 目标场次当前 `STORYBOARD_TABLE: PASS`，且所用行均 `PASS` |
| P4 | `STORYBOARD_IMAGE` | 同镜头当前 `STORYBOARD_PROMPT: PASS` |
| P5 | `APPROVED_STORYBOARD_SET` | 所需镜头当前 `STORYBOARD_IMAGE: PASS` |
| P6 | `VIDEO_PROMPT` | 同镜头当前图片本体 `PASS`；父集合逐镜条目 `APPROVED`、非 `STALE`，且精确图片版本一致 |
...
QA `PASS` 后：P1→P2、P2→P3、P3→P4、P4→P5、P6→P7。P4 的自动 QA `PASS` 只进入 P5，绝不直接产生 `APPROVED`。
...
P7 交付包只引用已经 `PASS` 的当前 VideoPromptSpec，并保留从视频 Prompt → 人工确认图片 → 分镜 Prompt → StoryboardRow → BEAT 的可追踪链。外部视频平台执行不在当前 Skill 集群的自动能力内时，交付可投喂提示词包并明确停点。
